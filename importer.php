...
function existing_email($existing_email, $data) {
	...
	if ($existing_email == $data[17]) {
		...
		return true;
	} else if ($existing_email == $data[41]) {
		//error_log(print_r("  match 33: " . $data[41], true));
		return true;
	}
	return false;
}
...
function grab_email($data) {
	if ($data[17]) {
		...
		//error_log(print_r("data[33]: " . $data[41], true));
		return $data[41];
	} else {
		return false;
	}
...
function type_to_list($user_id, $clubtec_type, $gender, $relationship)
{
	...
	switch ($tokens[0]) {
		...
		case 'Women':
			...
			error_log(print_r("relationship (1):  " . $relationship . ", gender: " . $gender, true));
									...
									switch ( $headers[ $i ] ){
										case 'TypeDesc':
											...
											if( !in_array( 'grp_name', $headers ) ) {
												type_to_list($user_id, $data[$i], $data[39], $data[5]);
											}
											/*												switch ($wp_role) {
                                                                                                case 'Non-Owner':
                                                                                                case 'Renters':
		...
		case 'homepage' :
			...
			<div class="wrap">
				...
				<div style="float:left; width:80%;">
					<h2><?php _e( 'Import users from ClubTec CSV (v2.08, Sep 9, 2017)','import-users-from-csv-with-meta' ); ?></h2>
				</div>
