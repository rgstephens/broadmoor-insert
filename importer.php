		...
		<table>
			...
			$row++;
						...
						if( !empty( $data ) ){
							...
							elseif( in_array( $headers[ $i ], $buddypress_fields ) ){ // buddypress
								...
								if( $data[ $i ] === '' ){
									...
									if ($headers[$i] == "grp_name") {
										//echo "grp_name: " . $data[$i];
										$lists = explode(',',$data[$i]);
										foreach ($lists as $value) {
											$listname = str_replace('.', '', str_replace('-', '', str_replace('\'', '', str_replace(' ', '', $value))));
											$return = update_user_meta( $user_id, $listname, true );
											if ($return) {
												error_log(print_r('update_user_meta success return: ' . $return . ', userId: ' . $user_id . 'listname: ' . $listname, true));
											} else {
												error_log(print_r('update_user_meta failed return: ' . $return . ', userId: ' . $user_id . 'listname: ' . $listname, true));
											}
											//update_user_meta( $user_id, $listname . 'ascii', "yes" );
											//error_log(print_r('return: ' . $return . ', userId: ' . $user_id . 'listname: ' . $listname  . "ascii", true));
										}
									} else {
				...
				/*					$all_meta_for_user = get_user_meta( $user_id );
                                    print_r( $user_id . ': ' . $all_meta_for_user . '<br>' );

                                    array_filter( array_map( function( $a ) {
                                        //return $a[0];
                                        print_r('value: ' . $a[0] . ' = ' . $a[1] . '<br>');
                                    }, $all_meta_for_user ) );*/
				/*
                 * 					$all_meta_for_user = array_map( function( $a ) {
                                        print_r($a . ', $a[0]: ' . $a[0] . '<br>')
                                        return $a[0];
                                    }, $all_meta_for_user );
                */
		...
		case 'homepage' :
			...
			<div class="wrap">
				...
				<div style="float:left; width:80%;">
					<h2><?php _e( 'Import users from CSV (v1.1)','import-users-from-csv-with-meta' ); ?></h2>
				</div>
